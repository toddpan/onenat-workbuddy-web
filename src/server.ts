#!/usr/bin/env node
/**
 * @dsh-external/onenat-workbuddy - 独立部署入口（不依赖 DSH）
 *
...
选项:
  -p, --port <n>              监听端口（默认 3081；env PORT）
      ...
      --token <token>         工具 HTTP 通道访问令牌（env WORKBUDDY_TOKEN；留空不校验）
...
export function createApp(cfg: StandaloneConfig): StandaloneApp {
  ...
  const tools = createWorkBuddyToolDefs({ store, directory, resolver, composer, engine, sshStore, consoleUrl })
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === '/healthz' || (cfg.prefix && url.pathname === `${cfg.prefix}/healthz`)) {
        sendJson(res, 200, {
          ok: true,
          service: '@dsh-external/onenat-workbuddy',
          version: pkgVersion(),
          standalone: true,
          authRequired: true,
      ...
      // ---------- 访问控制：控制台与业务 API 均需登录（工具通道可用 --token 令牌替代） ----------
      if (underPrefix && !sessionUser) {
        const isConsoleGet = method === 'GET' && (url.pathname === prefix || url.pathname === `${prefix}/` || url.pathname === `${prefix}/console`)
        const toolPath0 = `${prefix}/api/tools`
        const isToolsPath = url.pathname === toolPath0 || url.pathname.startsWith(toolPath0 + '/')
        const toolTokenOk = isToolsPath && authorizeTool(cfg, req, url)
        if (!toolTokenOk) {
          if (isConsoleGet) {
            // 未登录访问控制台 → 登录页
            res.statusCode = 200
      ...
      // ---------- 工具 HTTP 通道（登录会话 或 --token 令牌二选一） ----------
      const toolPath = `${prefix}/api/tools`
      if (url.pathname === toolPath || url.pathname.startsWith(toolPath + '/')) {
        if (!sessionUser && !authorizeTool(cfg, req, url)) {
          sendJson(res, 401, { ok: false, error: '工具通道未授权（需登录会话或 Authorization: Bearer <token>）' })
          return
        }
...
function authorizeTool(cfg: StandaloneConfig, req: http.IncomingMessage, url: URL): boolean {
  if (!cfg.apiToken) return true
  const auth = String(req.headers.authorization || '')
  if (auth === `Bearer ${cfg.apiToken}`) return true
  if (String(req.headers['x-workbuddy-token'] || '') === cfg.apiToken) return true
  return url.searchParams.get('token') === cfg.apiToken
}
...
async function main(): Promise<void> {
  ...
  if (cfg.apiToken) console.log('  ├─ 工具令牌    已启用（Authorization: Bearer ***）')
